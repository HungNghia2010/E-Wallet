    ...
    db.query('SELECT * FROM account WHERE id = ?',[req.user.id],(err,result) => {
        if(tongtien > result[0].money){
            ...
            for (let j = 0 ; j < test.length; j++){
                ...
                db.query('INSERT INTO trading_card SET ?',{ma_Giao_Dich : ma_Giao_Dich , ma_Khach_Hang: req.user.id , card_seri : test[j].seri , ma_The : test[j].the , card_type : test[j].loai , day_trading : day_trading , time_trading : time_trading , price : test[j].gia, trading_type : 'Thẻ cào', trading_status : 'Thành công'},(error)=>{
                    if (error){
                        console.log(error);
                    }else{
